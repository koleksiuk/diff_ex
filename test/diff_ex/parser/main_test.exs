defmodule DiffEx.Parser.MainTest do
  ...
  test "parses correctly simple-patch.diff and returns array of files", %{ commit_1: commit } do
    ...
    assert file_1.new_path == "test.txt"
    assert file_1.old_path == "/dev/null"
    assert file_1.body == [
      "@@ -0,0 +1 @@",
      "+This is file content"
    ]

    assert file_2.new_path == "simple-file.rb"
    assert file_2.old_path == "/dev/null"
    assert file_2.body == [
      "@@ -0,0 +1,3 @@",
      "+class Foo",
      "+  attr_accessor :bar",
  ...
  test "parses correctly commit-with-line-changes.diff and returns array of files", %{ commit_2: commit_2 } do
    ...
    assert file_1.new_path == "simple-file.rb"
    assert file_1.body == [
      "@@ -1,3 +1,4 @@",
      " class Foo",
      "-  attr_accessor :bar",
